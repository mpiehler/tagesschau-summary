...
export default function HomeClient({ initialSummaries }: { initialSummaries: Summary[] }) {
  const getDateStr = (iso: string) => iso.split('T')[0];
  const allDays = Array.from(new Set(initialSummaries.map(s => getDateStr(s.published_at))));
  const [selectedDateStr, setSelectedDateStr] = useState<string | null>(allDays[0] || null);
